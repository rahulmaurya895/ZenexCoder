...
export default function ReviewButton({ sourceId }) {
  ...
  async function refresh() {
    const list = await window.zezenexcoderr.review.list('pending_review');
  ...
  useEffect(() => {
    refresh().catch(() => {});
    const dispose = window.zezenexcoderr.review.onUpdate(refresh);
    return dispose;
  }, [sourceId]);
  ...
  async function openReview() {
    ...
    if (reviewMode === 'detached') {
      await window.zezenexcoderr.review.openDetached();
      return;
    }
    setOpen((value) => !value);
  }
