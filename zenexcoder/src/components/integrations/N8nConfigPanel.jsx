...
export default function N8nConfigPanel() {
  ...
  useEffect(() => {
    if (window.zezenexcoderr?.n8n) {
      setLoading(true);
      window.zezenexcoderr.n8n.getConfig()
        .then((saved) => {
          if (saved) setConfig(saved);
        })
        .finally(() => setLoading(false));

      const unbindStatus = window.zezenexcoderr.n8n.onStatusChanged((data) => {
        setStatus(data);
      });
      return () => unbindStatus?.();
    }
  ...
  async function handleSave() {
    setLoading(true);
    try {
      await window.zezenexcoderr.n8n.saveConfig(config);
  ...
  async function handleTestWebhook() {
    ...
    try {
      const res = await window.zezenexcoderr.n8n.triggerWebhook({
    ...
    <div className="settings-section n8n-config-panel">
      ...
      <div className="form-group">
        ...
        <input
          type="text"
          className="settings-input"
          placeholder="webhook/zezenexcoderr-swarm-trigger"
