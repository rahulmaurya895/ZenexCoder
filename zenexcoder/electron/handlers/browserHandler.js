...
async function resolveLocator(input) {
  ...
  const byZenexCoderId = pageRef.locator(`[data-zezenexcoderr-browser-id="${cssEscape(raw)}"]`);
...
export async function browserGetDOM() {
  ...
  const snapshot = await pageRef.evaluate(() => {
    ...
    function ensureId(element) {
      if (!element.dataset.zezenexcoderrBrowserId) {
        counter += 1;
        element.dataset.zezenexcoderrBrowserId = `b${counter}`;
      }
      return element.dataset.zezenexcoderrBrowserId;
    }
