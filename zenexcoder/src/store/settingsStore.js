...
export const useSettingsStore = create((set, get) => ({
  ...
  async loadSettings() {
    set({ loading: true });
    try {
      const saved = await window.zezenexcoderr.store.get('settings', defaultSettings);
      set({
        ...defaultSettings,
        ...saved,
        apiKeys: {
  ...
  async saveSettings(partial) {
    const saved = await window.zezenexcoderr.store.get('settings', {}).catch(() => ({}));
    ...
    await window.zezenexcoderr.store.set('settings', serializable);
    set(partial);
  },
  ...
  async resetSettings() {
    await window.zezenexcoderr.app.factoryReset();
    set(defaultSettings);
  }
}));
