...
export function useChat() {
  ...
  const sendMessage = useCallback(
    async (content, attachments = []) => {
      ...
      const pluginState = await window.zezenexcoderr.store.get('plugins', { customPlugins: [], enabled: {} }).catch(() => ({ customPlugins: [], enabled: {} }));
      ...
      const request = window.zezenexcoderr.ai.stream(
        {
            ...
            if (targetFolder && output) {
              ...
              for (const block of blocks) {
                ...
                if (fileName && block.code) {
                  ...
                  try {
                    await window.zezenexcoderr.file.write(targetPath, block.code);
                    if (projectPath) {
