...
export const useCollaborationStore = create((set, get) => ({
  ...
  async load() {
    const [collab, vault] = await Promise.all([
      window.zezenexcoderr.collab.list().catch(() => ({ peers: [], status: {} })),
      window.zezenexcoderr.collab.vaultStatus({ projectPath: get().projectPath() }).catch((error) => ({ ok: false, error: error.message }))
    ]);
    set({
      peers: collab.peers || [],
  ...
  async connect() {
    ...
    try {
      const result = await window.zezenexcoderr.collab.connect({ projectPath: get().projectPath() });
      set({
        connected: true,
        peers: result.peers || [],
  ...
  async disconnect() {
    await window.zezenexcoderr.collab.disconnect();
    set((state) => ({ connected: false, status: { ...state.status, connected: false } }));
  },

  async setVaultSecret(secret) {
    const vault = await window.zezenexcoderr.collab.setVaultSecret({ projectPath: get().projectPath(), secret });
    set({ vault });
    return vault;
  },

  async syncRules() {
    const result = await window.zezenexcoderr.collab.syncRules();
    set({ status: result.status || get().status });
    return result;
  },

  async muteOrigin(originNodeId, muted = true) {
    await window.zezenexcoderr.collab.muteOrigin({ originNodeId, muted });
    set((state) => ({
  ...
  updatePresence(file, status = 'Coding') {
    return window.zezenexcoderr.collab.updatePresence({ file, status }).catch(() => {});
  },
