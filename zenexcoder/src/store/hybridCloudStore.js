...
export const useHybridCloudStore = create((set, get) => ({
  ...
  async loadConfig() {
    set({ loading: true });
    try {
      if (typeof window !== 'undefined' && window.zezenexcoderr?.store) {
        const saved = await window.zezenexcoderr.store.get('hybrid_cloud_config', defaultHybridConfig);
  ...
  async updateConfig(patch) {
    ...
    try {
      if (typeof window !== 'undefined' && window.zezenexcoderr?.store) {
        await window.zezenexcoderr.store.set('hybrid_cloud_config', {
