...
export const useClusterStore = create((set, get) => ({
  ...
  async load() {
    ...
    try {
      const state = await window.zezenexcoderr.cluster.list();
      set({
  ...
  async scanStart() {
    ...
    try {
      const result = await window.zezenexcoderr.cluster.scanStart();
  ...
  async requestPair(node) {
    ...
    await window.zezenexcoderr.cluster.requestPair({ nodeId: node.nodeId, ip: node.ip, port: node.port });
  },

  async verifyPin(pin) {
    const node = get().pairingNode;
    if (!node) return null;
    const result = await window.zezenexcoderr.cluster.verifyPin({ nodeId: node.nodeId, ip: node.ip, pin });
  ...
  async disconnect(nodeId) {
    await window.zezenexcoderr.cluster.disconnect(nodeId);
    set((state) => ({
  ...
  async setRouting(patch) {
    const state = await window.zezenexcoderr.cluster.setRouting(patch);
