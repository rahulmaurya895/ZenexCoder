...
export default function ShellSelector({ onShellChanged }) {
  ...
  async function refresh() {
    ...
    setError('');
    try {
      const result = await window.zezenexcoderr.terminal.getShells();
  ...
  useEffect(() => {
    refresh().catch(() => {});
    const dispose = window.zezenexcoderr.terminal.onShellChanged((result) => {
      setShells(result.shells || []);
      setSelected(result.selected || '');
  ...
  async function applyShell(shellPath) {
    ...
    setError('');
    try {
      const result = await window.zezenexcoderr.terminal.setShell(shellPath.trim());
      setShells(result.shells || []);
