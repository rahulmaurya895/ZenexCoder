...
function loadAppSettings() {
  const storeFile = path.join(app.getPath('userData'), 'zezenexcoderr-secure.json');
  try {
...
async function createHealingWorktree(incident, settings = {}, job) {
  ...
  const worktreeRoot = path.join(projectPath, '.zezenexcoderr_worktrees');
...
async function cleanupFailedWorktree(incident, job) {
  ...
  const expectedRoot = path.join(job.projectPath, '.zezenexcoderr_worktrees');
