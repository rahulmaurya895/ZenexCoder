...
export default function GitSyncBar() {
  ...
  useEffect(() => {
    window.zezenexcoderr?.github?.tokenStatus?.().then((res) => setHasToken(Boolean(res?.hasToken))).catch(() => {});
  }, []);

  async function saveToken() {
    if (!githubToken.trim()) return;
    await window.zezenexcoderr.github.saveToken(githubToken.trim());
    setHasToken(true);
    setShowTokenInput(false);
    setGithubToken('');
    window.zezenexcoderr.notify.show({ title: 'GitHub Token', body: 'Token saved securely.' });
  }
  ...
  async function run(action, fn, notifyBody) {
    ...
    try {
      ...
      await window.zezenexcoderr.notify.show({ title: 'Git', body: notifyBody });
      ...
      await window.zezenexcoderr.notify.show({ title: 'Git error', body: error.message });
      return null;
    }
  }
