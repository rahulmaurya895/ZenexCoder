...
export const useProjectStore = create((set, get) => ({
  ...
  async openProject(folderPath) {
    if (!folderPath) {
      const chosen = await window.zezenexcoderr.folder.openDialog();
      if (!chosen) {
        return null;
      }
      folderPath = chosen;
    ...
    set({ loading: true });
    try {
      const fileTree = await window.zezenexcoderr.folder.readTree(folderPath);
      await window.zezenexcoderr.db.upsertProject({ path: folderPath, name: basename(folderPath) });
      const projects = await window.zezenexcoderr.db.listProjects();
  ...
  async refreshTree() {
    ...
    const fileTree = await window.zezenexcoderr.folder.readTree(projectPath);
    set({ fileTree });
  },
  ...
  async saveFile(id) {
    ...
    await window.zezenexcoderr.file.write(file.path, file.content);
    set((state) => ({
  ...
  async setProjectWorkMode(workMode) {
    ...
    await window.zezenexcoderr.db.upsertProject({ path: projectPath, name: basename(projectPath), settings });
    set({ projectSettings: settings });
  },
  async writeNewFile(filePath, content) {
    await window.zezenexcoderr.file.write(filePath, content);
    await get().refreshTree();
