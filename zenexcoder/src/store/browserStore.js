...
export const useBrowserStore = create((set, get) => ({
  ...
  async refreshState() {
    const state = await window.zezenexcoderr.browser.state();
    set((current) => ({
      ...current,
      ...state,
  ...
  async start() {
    ...
    try {
      const state = await window.zezenexcoderr.browser.start();
  ...
  async stop() {
    const state = await window.zezenexcoderr.browser.stop();
  ...
  async navigate(url) {
    set({ isLoading: true, error: '' });
    try {
      const result = await window.zezenexcoderr.browser.navigate(url);
  ...
  async back() {
    const result = await window.zezenexcoderr.browser.back();
    set((state) => ({ ...state, ...result, dom: result.dom || state.dom }));
    return result;
  },
  async forward() {
    const result = await window.zezenexcoderr.browser.forward();
    set((state) => ({ ...state, ...result, dom: result.dom || state.dom }));
    return result;
  },
  async reload() {
    const result = await window.zezenexcoderr.browser.reload();
    set((state) => ({ ...state, ...result, dom: result.dom || state.dom }));
    return result;
  },
  async readPage() {
    set({ isLoading: true, error: '' });
    try {
      const dom = await window.zezenexcoderr.browser.getDOM();
