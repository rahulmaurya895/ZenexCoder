...
export function registerAgentHandlers() {
  ...
  ipcMain.handle('agent:approval-request', async (_event, payload = {}) => {
    ...
    sendToAll('agent:approval-pending', action);
    showNotification('ZenexCoder approval needed', action.description || action.title || 'Approval requested', {
      type: 'approval',
      id: action.id
    });
    return action;
  ...
  ipcMain.handle('notify:show', async (_event, payload = {}) => {
    showNotification(payload.title || 'ZenexCoder', payload.body || '', payload);
    return { ok: true };
  });
}
