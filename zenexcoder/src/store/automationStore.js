...
export const useAutomationStore = create((set, get) => ({
  ...
  async loadAutomations() {
    const automations = await window.zezenexcoderr.automation.list();
    set({ automations });
  },
  async addAutomation(automation) {
    const saved = await window.zezenexcoderr.automation.save(automation);
  ...
  async updateAutomation(id, patch) {
    ...
    const saved = await window.zezenexcoderr.automation.save({ ...current, ...patch, id });
    set((state) => ({ automations: state.automations.map((item) => (item.id === id ? saved : item)) }));
    return saved;
  },
  async deleteAutomation(id) {
    await window.zezenexcoderr.automation.delete(id);
  ...
  async runAutomation(id, context = {}) {
    ...
    const updated = await window.zezenexcoderr.automation.markRun(id);
