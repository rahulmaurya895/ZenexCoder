...
export const useWindowStore = create((set, get) => ({
  ...
  async loadPopoutState() {
    ...
    try {
      const state = await window.zezenexcoderr.window.getPopoutState();
  ...
  async togglePopout() {
    set({ loading: true, error: '' });
    try {
      const state = await window.zezenexcoderr.window.togglePopout();
      set((current) => ({ ...normalizePopoutState(state, current), loading: false, error: '' }));
      return state;
    } catch (error) {
      set({ loading: false, error: error.message });
  ...
  async setPopoutHotkey(hotkey) {
    set({ loading: true, error: '' });
    try {
      const state = await window.zezenexcoderr.window.setPopoutHotkey(hotkey);
      set((current) => ({ ...normalizePopoutState(state, current), loading: false, error: '' }));
      return state;
    } catch (error) {
      set({ loading: false, error: error.message });
