...
export const usePipelineStore = create((set, get) => ({
  ...
  async load() {
    const state = await window.zezenexcoderr.cicd.getState().catch(() => null);
    if (state) {
      set({
  ...
  async generateIaC() {
    ...
    try {
      const result = await window.zezenexcoderr.cicd.generateIaC({
  ...
  async startDeploy() {
    set({ loading: true, error: '' });
    try {
      const result = await window.zezenexcoderr.cicd.deployStart({
        projectPath: get().projectPath(),
        provider: get().provider,
        target: get().target,
        dryRun: get().dryRun,
  ...
  async rollback() {
    set({ loading: true, error: '' });
    try {
      const result = await window.zezenexcoderr.cicd.rollbackManual({
