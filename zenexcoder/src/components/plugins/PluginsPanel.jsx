...
export default function PluginsPanel() {
  ...
  useEffect(() => {
    window.zezenexcoderr.store.get('plugins', { enabled: {}, customPlugins: [] }).then((saved) => {
      setEnabled(saved.enabled || {});
      setCustomPlugins(saved.customPlugins || []);
    });
    window.zezenexcoderr.db.listSnippets().then(setSnippets).catch(() => {});
  }, []);

  async function persist(nextEnabled = enabled, nextCustom = customPlugins) {
    await window.zezenexcoderr.store.set('plugins', { enabled: nextEnabled, customPlugins: nextCustom });
  }
  ...
  async function generateCommitMessage() {
    let diff = '';
    const runner = window.zezenexcoderr.terminal.run(
  ...
  async function exportPdf() {
    ...
    await window.zezenexcoderr.export.chatPdf({
