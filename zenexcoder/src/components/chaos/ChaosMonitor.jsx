...
export default function ChaosMonitor() {
  ...
  useEffect(() => {
    if (window.zezenexcoderr?.chaos) {
      window.zezenexcoderr.chaos.getLogs().then((res) => {
        if (res) {
          setLogs(res.logs || []);
      ...
      const unbindStatus = window.zezenexcoderr.chaos.onStatusChanged((data) => {
        if (data?.testRun) {
          setActiveRuns((prev) => {
      ...
      const unbindLog = window.zezenexcoderr.chaos.onLog((logEntry) => {
