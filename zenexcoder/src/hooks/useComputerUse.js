...
function terminateAgent(reason = 'AI CONTROL TERMINATED') {
  ...
  window.zezenexcoderr.ai?.abortAll?.(reason).catch(() => {});
  window.zezenexcoderr.notify?.show?.({ title: 'AI CONTROL TERMINATED', body: reason }).catch(() => {});
}

function ensureComputerListeners() {
  if (!window.zezenexcoderr?.computer) return;
  if (!disposers.length) {
    disposers = [
      window.zezenexcoderr.computer.onStateChanged((payload) => useComputerStore.getState().applyState(payload)),
      window.zezenexcoderr.computer.onActionLogged((payload) => useComputerStore.getState().addLog(payload)),
      window.zezenexcoderr.computer.onEmergencyStop((payload) => {
