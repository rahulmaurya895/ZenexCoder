...
export default function CodeEditor() {
  ...
  useEffect(() => {
    ...
    const disposers = [
      window.zezenexcoderr.app.onMenu('menu:save-file', save),
      window.zezenexcoderr.app.onMenu('menu:ai-complete', () => handleComplete()),
      window.zezenexcoderr.app.onMenu('menu:ai-explain', () => handleExplain()),
      window.zezenexcoderr.app.onMenu('menu:ai-fix', () => handleFix()),
      window.zezenexcoderr.app.onMenu('menu:ai-refactor', () => handleRefactor()),
      window.zezenexcoderr.app.onMenu('menu:ai-tests', () => handleTests())
    ];
  ...
  async function handleComplete() {
    ...
    await completeCode({
      ...
      after,
      language,
      onToken: (token) => editor.trigger('zezenexcoderr', 'type', { text: token })
    });
  }
  ...
  async function handleDocs() {
    ...
    await window.zezenexcoderr.review.add({
  ...
  async function handleTests() {
    ...
    const created = await window.zezenexcoderr.file.read(path).catch(() => ({ content: '' }));
    await window.zezenexcoderr.review.add({
      sessionId: activeSessionId,
      filePath: path,
      beforeContent: '',
