...
export default function ShadowTrainerUI() {
  ...
  useEffect(() => {
    if (window.zezenexcoderr?.shadowAI) {
      window.zezenexcoderr.shadowAI.getProfile().then((p) => {
        if (p) {
          setProfile(p);
      ...
      const unbind = window.zezenexcoderr.shadowAI.onStatusChanged((data) => {
        setStatus(data);
      });
      return () => unbind?.();
    }
  ...
  async function handleStartTraining() {
    ...
    try {
      const res = await window.zezenexcoderr.shadowAI.train(reposList, token);
      if (res?.profile) {
