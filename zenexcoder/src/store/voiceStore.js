...
export const useVoiceStore = create((set, get) => ({
  ...
  async loadSettings() {
    const saved = await window.zezenexcoderr.store.get('voice:settings', defaultVoiceSettings).catch(() => defaultVoiceSettings);
    set({
      settings: {
  ...
  async saveSettings(patch = {}) {
    ...
    await window.zezenexcoderr.store.set('voice:settings', settings);
    set({ settings });
    return settings;
  },
  ...
  async connect() {
    ...
    try {
      const state = await window.zezenexcoderr.voice.connect({
        ...settings,
        apiKey,
        projectPath
      });
      ...
      window.zezenexcoderr.notify.show({
  ...
  async disconnect() {
    await window.zezenexcoderr.voice.disconnect().catch(() => {});
    set({
      ...disconnectedState,
      loading: false
    });
  ...
  sendContextUpdate(payload = {}) {
    ...
    return window.zezenexcoderr.voice.sendContextUpdate(payload).catch((error) => ({ ok: false, error: error.message }));
  }
}));
