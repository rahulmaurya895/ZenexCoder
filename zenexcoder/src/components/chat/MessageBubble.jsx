...
export default function MessageBubble({ message, highlight = false }) {
  ...
  useEffect(() => {
    window.zezenexcoderr.store.get('plugins', { enabled: {} }).then(setPluginState).catch(() => {});
  }, []);
  ...
  async function saveFileToProject(code, language) {
    ...
    if (projectPath) {
      ...
      await window.zezenexcoderr.file.write(fullPath, code);
  ...
  async function saveSnippet(code, language) {
    await window.zezenexcoderr.db.addSnippet({
      type: 'message-code',
