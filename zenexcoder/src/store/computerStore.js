...
export const useComputerStore = create((set, get) => ({
  ...
  async refreshState() {
    const state = await window.zezenexcoderr.computer.state();
    get().applyState(state);
    return state;
  },
  async setEnabled(enabled) {
    const state = await window.zezenexcoderr.computer.setEnabled(enabled);
    get().applyState(state);
    return state;
  },
  async setUnattended(allowUnattended) {
    const state = await window.zezenexcoderr.computer.setUnattended(allowUnattended);
    get().applyState(state);
    return state;
  },
  async unlock() {
    const state = await window.zezenexcoderr.computer.unlock();
    get().applyState(state);
    return state;
  },
  async lock(reason = 'manual') {
    const state = await window.zezenexcoderr.computer.lock(reason);
    get().applyState(state);
    return state;
  },
  async getScreen() {
    set({ error: '' });
    try {
      const screen = await window.zezenexcoderr.computer.getScreen();
