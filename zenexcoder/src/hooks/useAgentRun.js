...
function ensureAgentListeners() {
  if (!window.zezenexcoderr?.agent) return;
  if (!disposers.length) {
    disposers = [
      window.zezenexcoderr.agent.onStepUpdate(({ step }) => useAgentStore.getState().updateStep(step)),
      window.zezenexcoderr.agent.onRunUpdate((payload) => {
      ...
      window.zezenexcoderr.agent.onApprovalPending((approval) => {
        useAgentStore.getState().addApproval(approval);
        useAppStore.getState().setRightPanelOpen(true);
      }),
      window.zezenexcoderr.agent.onApprovalResolved((payload) => {
