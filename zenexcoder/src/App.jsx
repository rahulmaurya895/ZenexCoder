...
function MainApp() {
  ...
  useEffect(() => {
    const disposers = [
      window.zezenexcoderr.app.onMenu('menu:open-folder', () => useProjectStore.getState().openProject()),
      window.zezenexcoderr.app.onMenu('menu:save-file', () => activeFileId && saveFile(activeFileId))
    ];
  ...
  useEffect(() => {
    const disposers = [
      window.zezenexcoderr.automation.onTrigger((payload) => {
  ...
  useEffect(() => {
    ...
    window.addEventListener('zezenexcoderr:open-settings', openSettings);
    return () => window.removeEventListener('zezenexcoderr:open-settings', openSettings);
  }, []);
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const dispose = window.zezenexcoderr?.notify?.onShow?.((payload) => addNotification(payload));
    return () => dispose?.();
