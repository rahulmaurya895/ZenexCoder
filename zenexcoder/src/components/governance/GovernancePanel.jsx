...
export default function GovernancePanel() {
  ...
  useEffect(() => {
    window.zezenexcoderr.audit.getLogs().then(setLogs).catch(() => {});
    window.zezenexcoderr.policy.getStatus().then(setPolicy).catch(() => {});
  }, []);

  async function toggle(ruleId, enabled) {
    const next = await window.zezenexcoderr.policy.updateSetting({ ruleId, enabled });
    setPolicy(next);
  }
