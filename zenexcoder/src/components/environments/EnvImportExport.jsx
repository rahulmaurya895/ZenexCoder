...
export default function EnvImportExport({ env, projectPath, store }) {
  ...
  async function importFile() {
    ...
    if (!filePath) return;
    const raw = await window.zezenexcoderr.env.readDotFile(filePath);
  ...
  async function exportFile() {
    ...
    const result = await runWithApproval(
      ...
      () => window.zezenexcoderr.env.writeDotFile(target, content)
    );
    if (!result) return;
    await window.zezenexcoderr.notify.show({
      title: '.env exported',
