...
function TreeNode({ node, depth = 0 }) {
  ...
  async function readNodeFile() {
    const result = await window.zezenexcoderr.file.read(node.path);
        ...
        <div className="context-menu" style={{ left: menu.x, top: menu.y }} onMouseLeave={() => setMenu(null)}>
          ...
          <button
            onClick={async () => {
              ...
              if (target) {
                await window.zezenexcoderr.file.rename(node.path, target);
                await refreshTree();
              }
            }}
          >
          ...
          <button
            onClick={async () => {
              if (window.confirm(`Delete ${node.name}?`)) {
                await window.zezenexcoderr.file.delete(node.path);
                await refreshTree();
              }
            }}
          >
