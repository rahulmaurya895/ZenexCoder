...
export const useAgentStore = create((set, get) => ({
  ...
  async startRun(plan, options = {}) {
    ...
    return window.zezenexcoderr.agent.startRun({ ...normalizedPlan, ...options });
  },
  hydrateRun(plan) {
  ...
  async pause() {
    ...
    set({ runState: 'paused' });
    if (runId) await window.zezenexcoderr.agent.control({ runId, action: 'pause' });
  },
  async resume() {
    const runId = get().plan.id;
    set({ runState: 'running' });
    if (runId) await window.zezenexcoderr.agent.control({ runId, action: 'resume' });
  },
  async stop() {
    const runId = get().plan.id;
    set({ runState: 'stopped' });
    if (runId) await window.zezenexcoderr.agent.control({ runId, action: 'stop' });
  },
  async skipStep(id) {
    const runId = get().plan.id;
    set((state) => ({
    ...
    if (runId) await window.zezenexcoderr.agent.control({ runId, action: 'skip', stepId: id });
  },
  editStep(id, patch) {
    const runId = get().plan.id;
    set((state) => ({
    ...
    if (runId) {
      window.zezenexcoderr.agent.control({ runId, action: 'edit-step', stepId: id, patch }).catch(() => {});
    }
  },
  ...
  async resolveApproval(id, decision, options = {}) {
    ...
    await window.zezenexcoderr.agent.respondApproval({
      actionId: id,
      decision,
    ...
    if (approval) await window.zezenexcoderr.store.set(`approval:last:${id}`, { approval, decision }).catch(() => {});
  },
  ...
  insertSteerStep(content) {
    ...
    if (runId && insertedStep) {
      window.zezenexcoderr.agent.control({ runId, action: 'insert-step', step: insertedStep }).catch(() => {});
    }
  },
