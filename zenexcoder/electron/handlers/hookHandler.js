...
const ZEZENEXCODERR_BEGIN = '# >>> ZenexCoder managed hook >>>';
const ZEZENEXCODERR_END = '# <<< ZenexCoder managed hook <<<';
...
PORT_FILE="$HOOK_DIR/../.zezenexcoderr-port"
if [ ! -f "$PORT_FILE" ]; then
  exit 0
fi
...
HTTP_CODE=$(curl -sS -o /tmp/zezenexcoderr-hook-response-$$.json -w "%{http_code}" --connect-timeout 1 --max-time 185 -H "Content-Type: application/json" -X POST --data "$PAYLOAD" "http://127.0.0.1:$PORT/webhook" 2>/dev/null)
CURL_STATUS=$?
rm -f /tmp/zezenexcoderr-hook-response-$$.json
...
async function writePortFile(projectPath) {
  ...
  await fs.writeFile(path.join(gitDir, '.zezenexcoderr-port'), String(port), 'utf8');
  return { gitDir, port };
}
...
async function installGitHook(projectPath, hookType) {
  ...
  const backupPath = `${hookPath}.zezenexcoderr-backup`;
  await fs.mkdir(hooksDir, { recursive: true });

  if (await exists(hookPath)) {
    const existing = await fs.readFile(hookPath, 'utf8').catch(() => '');
    if (!existing.includes(ZEZENEXCODERR_BEGIN) && !(await exists(backupPath))) {
...
async function removeGitHook(projectPath, hookType) {
  ...
  const existing = await fs.readFile(hookPath, 'utf8').catch(() => '');
  if (!existing.includes(ZEZENEXCODERR_BEGIN)) {
...
async function listInstalledGitHooks(projectPath) {
  ...
  try {
    ...
    for (const hookType of VALID_GIT_HOOKS) {
      ...
      result[hookType] = {
        installed: script.includes(ZEZENEXCODERR_BEGIN),
        hookPath
      };
    }
  } catch {
...
export async function cleanupHookPortFiles(projectPaths = null) {
  ...
  for (const projectPath of paths.filter(Boolean)) {
    try {
      const gitDir = await resolveGitDir(projectPath);
      await fs.rm(path.join(gitDir, '.zezenexcoderr-port'), { force: true });
    } catch {
