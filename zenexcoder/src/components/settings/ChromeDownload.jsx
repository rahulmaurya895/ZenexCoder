...
export default function ChromeDownload() {
  ...
  const installChrome = async () => {
    ...
    try {
      ...
      await window.zezenexcoderr.app.runCommand('npx playwright install chrome');
      // Retrieve Chrome executable path via Playwright API
      const result = await window.zezenexcoderr.app.runCommand(`node -e "console.log(require('playwright').chromium.executablePath())"`);
