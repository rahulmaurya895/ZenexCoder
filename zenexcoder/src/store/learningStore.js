...
export const useLearningStore = create((set, get) => ({
  ...
  async load() {
    ...
    try {
      const [rules, stats, analysisState] = await Promise.all([
        window.zezenexcoderr.learning.getRules(),
        window.zezenexcoderr.learning.getStats(),
        window.zezenexcoderr.learning.getAnalysisState()
      ]);
  ...
  async saveRule(rule) {
    const saved = await window.zezenexcoderr.learning.updateRule(rule);
    set((state) => ({
  ...
  async deleteRule(id) {
    await window.zezenexcoderr.learning.deleteRule(id);
    set((state) => ({ rules: state.rules.filter((rule) => rule.id !== id) }));
    await get().refreshStats();
  },

  async triggerAnalysis() {
    const result = await window.zezenexcoderr.learning.triggerAnalysis();
  ...
  async refreshStats() {
    const stats = await window.zezenexcoderr.learning.getStats();
    set({ stats });
    return stats;
  },
