...
export function useDictation({ language = 'en-US', onFinalTranscript } = {}) {
  ...
  const startListening = useCallback(() => {
    ...
    if (!SpeechRecognition) {
      ...
      setError(message);
      window.zezenexcoderr?.notify?.show?.({ title: 'Dictation unavailable', body: message, type: 'warning' }).catch(() => {});
      return false;
    }
    ...
    try {
      ...
      recognition.onerror = (event) => {
        ...
        setError(message);
        window.zezenexcoderr?.notify?.show?.({ title: 'Dictation error', body: message, type: 'error' }).catch(() => {});
      };
