  ...
  return (
    <article className={`incident-card ${incident.status === 'manual_required' ? 'needs-review' : ''}`}>
      ...
      <div className="incident-actions">
        {incident.url ? (
          <button type="button" onClick={() => window.zezenexcoderr.app.openExternal(incident.url)}>
            <ExternalLink size={14} /> View on Sentry
          </button>
        ) : null}
        {incident.prUrl ? (
          <button type="button" className="primary-button" onClick={() => window.zezenexcoderr.app.openExternal(incident.prUrl)}>
